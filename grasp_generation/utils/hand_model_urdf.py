...
from urdf_parser_py.urdf import Robot, Box, Sphere, Cylinder
...
class HandModel:
    def __init__(self, urdf_path, contact_points_path, default_pos, n_surface_points=0, device='cpu'):
        ...
        for link in self.robot.links:
            ...
            collision_link_vertices = []
            collision_link_faces = []
            collision_n_link_vertices = 0
            visual_link_vertices = []
            visual_link_faces = []
            visual_n_link_vertices = 0
            for collision in link.collisions:
                if type(collision.geometry) == Sphere:
                    ...
                    link_mesh = tm.primitives.Cylinder(radius=collision.geometry.radius, height=collision.geometry.length)
                ...
                collision_link_vertices.append(vertices)
                collision_link_faces.append(faces + collision_n_link_vertices)
                collision_n_link_vertices += len(vertices)
                
                
                # load visual mesh
                visual = link.visual
                
                if type(visual.geometry) == Sphere:
                    link_mesh = tm.primitives.Sphere(radius=visual.geometry.radius)
                    self.mesh[link.name]['radius'] = visual.geometry.radius
                elif type(visual.geometry) == Box:
                    # link_mesh = tm.primitives.Box(extents=visual.geometry.size)
                    link_mesh = tm.load_mesh(os.path.join(os.path.dirname(urdf_path), 'meshes', 'box.obj'), process=False)
                    link_mesh.vertices *= np.array(visual.geometry.size) / 2
                elif type(visual.geometry) == Cylinder:
                    link_mesh = tm.primitives.Cylinder(radius=visual.geometry.radius, height=visual.geometry.length)
                else:
                    filename = os.path.join(os.path.dirname(urdf_path), visual.geometry.filename)
                ...
                vertices = vertices * scale
                vertices = vertices @ rotation.T + translation
                visual_link_vertices.append(vertices)
                visual_link_faces.append(faces + collision_n_link_vertices)
                visual_n_link_vertices += len(vertices)


            collision_link_vertices = torch.cat(collision_link_vertices, dim=0)
            collision_link_faces = torch.cat(collision_link_faces, dim=0)
            #update collision mesh
            self.mesh[link.name].update({
                'vertices': collision_link_vertices,
                'faces': collision_link_faces,
            })
            if 'radius' not in self.mesh[link.name]:
                self.mesh[link.name]['face_verts'] = index_vertices_by_faces(collision_link_vertices, collision_link_faces)
            areas[link.name] = tm.Trimesh(collision_link_vertices.cpu().numpy(), collision_link_faces.cpu().numpy()).area.item()


            #update visual mesh
            visual_link_vertices = torch.cat(visual_link_vertices, dim=0)
            visual_link_faces = torch.cat(visual_link_faces, dim=0)
            self.mesh[link.name].update({
                'visual_vertices': visual_link_vertices,
                'visual_faces': visual_link_faces,
            })
        ...
        self.joints_lower = torch.tensor([joint.limit.lower for joint in self.robot.joints if joint.joint_type == 'revolute' or joint.joint_type == 'prismatic'], dtype=torch.float, device=device)
        self.joints_upper = torch.tensor([joint.limit.upper for joint in self.robot.joints if joint.joint_type == 'revolute' or joint.joint_type == 'prismatic'], dtype=torch.float, device=device)
        ...
        # hand_folder = "mjcf/franka_hand/"
        # contact_bound = json.load(open(hand_folder + "contact_bounds.json", 'r'))
        ...
        # file_path = hand_folder + 'contact_points.json'
        ...
        # self.adjacency_mask[self.link_name_to_link_index['bh282_hand_base'], self.link_name_to_link_index['bh282_perception_palm']] = True
        # self.adjacency_mask[self.link_name_to_link_index['bh282_perception_palm'], self.link_name_to_link_index['bh282_hand_base']] = True
